...
public interface CommentRepository extends JpaRepository<Comment, Long> {
    ...
    @Query("select c from Comment c left join fetch c.member " +
            "where c.post.deleted = false and c.deleted = false and c.post.id = :postId")
    List<Comment> findByPostIdFetchWithMember(@Param("postId") Long postId);

    @Query("select c from Comment c left join fetch c.member " +
            "where c.member.deleted = false and c.deleted = false and c.id = :id and c.member.id = :memberId")
    Optional<Comment> findByIdAndMemberIdFetchWithMember(@Param("id") Long id, @Param("memberId") Long memberId);
